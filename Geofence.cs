...
namespace TeslaLogger
{
    ...
    public class Geofence
    {
        ...
        internal void Init()
        {
            ...
            RacingMode = false;
            ReadGeofenceFile(geofenceList, FileManager.GeofenceFilename);
            Logfile.Log("Geofence: addresses inserted from geofence.csv: " + geofenceList.Count);
            if (!File.Exists(FileManager.GeofencePrivateFilename))
            {
                Logfile.Log("Create: " + FileManager.GeofencePrivateFilename);
                File.AppendAllText(FileManager.GeofencePrivateFilename, "");
            }

            UpdateTeslalogger.Chmod(FileManager.GeofencePrivateFilename, 666);
            ReadGeofenceFile(geofencePrivateList, FileManager.GeofencePrivateFilename);
                ...
                using (StreamReader file = new StreamReader(filename))
                {
                    while ((line = file.ReadLine()) != null)
                    {
                        try
                        {
                            ...
                            if (filename == FileManager.GeofencePrivateFilename)
                            {
